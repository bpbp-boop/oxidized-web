...
module Oxidized
  module API
    ...
    class Web
      ...
      DEFAULT_CONF_SEARCH_THREADS = 4
      MAX_CONF_SEARCH_THREADS = 16
      ...
      def initialize(nodes, configuration)
        ...
        WebApp.set :conf_search_mutex, Mutex.new
        WebApp.set :conf_search_threads, @configuration[:conf_search_threads]
      ...
      def run
        ...
        @thread = Thread.new do
          ...
          logger.info "Oxidized-web server listening on #{addr}:#{port} " \
                      ...
                      "config search threads: #{@configuration[:conf_search_threads]})"
          @server.run.join
        end
      end
      ...
      def self.parse_new_configuration(configuration)
        ...
        hide_node_vars = hide_node_vars.map(&:to_sym)
        {
          ...
          conf_search_threads: normalize_conf_search_threads(configuration.conf_search_threads?)
        }
      end
      ...
      def self.parse_legacy_configuration(configuration)
        ...
        end
        {
          ...
          conf_search_threads: DEFAULT_CONF_SEARCH_THREADS
        }
      end

      def self.normalize_conf_search_threads(value)
        threads = Integer(value || DEFAULT_CONF_SEARCH_THREADS)
        threads.clamp(1, MAX_CONF_SEARCH_THREADS)
      rescue ArgumentError, TypeError
        DEFAULT_CONF_SEARCH_THREADS
      end
